...
1. Claude Code fetches unresolved PR conversations
2. Claude Code creates one work-item per conversation in the tracker
3. User approves PR in web portal (conversations remain as work-items)
4. Claude Code creates feature worktrees to fix each work-item
    ...
    def _fetch_github_conversations(self, pr_number: int) -> list[dict]:
        ...
        try:
            remote_url = subprocess.check_output(
                ["git", "config", "--get", "remote.origin.url"], text=True, stderr=subprocess.PIPE
            ).strip()
        ...
        try:
            ...
            for thread in review_threads:
                ...
                conversations.append(
                    {
                        "id": thread["id"],
                        "url": first_comment.get(
                            "url", f"{pr_data['url']}#discussion_{thread['id']}"
                        ),
        ...
        try:
            ...
            if "not found" in error_msg.lower() and "label" in error_msg.lower():
                ...
                try:
                    ...
                    retry_msg = (
                        retry_error.stderr.strip() if retry_error.stderr else str(retry_error)
                    )
