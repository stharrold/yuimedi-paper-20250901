...
- **Renaming a file inside an `ARCHIVED/<package>/` that has a `MANIFEST.sha256` must update the manifest path too.** `f1bd5a6` renamed `96541.pdf`/`96541_modified.pdf` to date-prefixed names and left 2 dead entries (65/67 verified) until v6.0.0 fixed them. Re-run `shasum -a 256 -c MANIFEST.sha256` from inside the package after any rename.
...
- **Crossref returns JMIR *preprint* DOIs by default** (`10.2196/preprints.NNNNN`) when querying by title. Add `filter=type:journal-article` and check `container-title` to get the published DOI, then resolve it (`curl -s -o /dev/null -w '%{redirect_url}' https://doi.org/<doi>`) for the article URL.
...
- **Library layout is `../library/docs/<stem>/<stem>.pdf`** (plus `page_NNN.md`, `status.jsonl`). Library commit `d7fa638` (2026-01-20) moved everything from flat `docs/<stem>.pdf`, sometimes inserting a document type after the author (`_Report_`, `_Whitepaper_`, `_Guide_`, `_Web-Resource_`); a folder year can be the online-first date (`kamble2019` lives in `2018_Kamble_...`). From `docs/research/answers/`, links use `../../../library/docs/...`.
- **Sweep for stale library paths** (`ARCHIVED/` snapshots deliberately excluded). As of 2026-09-15 it prints exactly 1 line, the `YYYY_Author_Venue_Title/...pdf` naming placeholder in `docs/guides/scholar-labs-workflow.md`; anything else is a real stale path: `git grep -n -o -E '\.\./library/docs/[A-Za-z0-9_./-]+\.pdf' -- ':!ARCHIVED' | perl -ne 'chomp; ($f,$l,$p)=split(/:/,$_,3); print "$f:$l $p\n" unless -e $p'`
- **Resolving a moved library file:** `git -C ../library log --all --name-status -M -- docs/<old>.pdf` shows the commit that deleted it and what it added, often several candidates. Match by SHA-256 of the old content (`git cat-file -p <blob> | shasum -a 256`) against the current PDFs. Never compare `git hash-object` IDs: the library now stores PDFs in LFS, so identical files have different blob IDs. A document never in library history (e.g. an arXiv preprint) gets a canonical URL instead of a local path.
...
- **zsh does not word-split unquoted variables.** `F="a.md b.md"; git add $F` passes ONE path and fails "did not match any files" (a chained `grep -c` then reports a false 0). List paths literally or use an array: `F=(a.md b.md); git add $F`. Same trap, other shapes seen at v6.0.0: a command in a variable (`S="uv run scripts/secrets_run.py"; $S gh ...` fails "no such file or directory"), and `for id in $IDS` over a newline-separated list runs ONCE with all IDs as one argument, so a watch loop "fails" without checking anything. `${PIPESTATUS[0]}` is bash-only and expands empty in zsh (use `$pipestatus[1]`).
- **The Read tool needs a real image extension.** A JPEG saved as `.img` is dumped as binary text; copy it to `.jpg` first.
- **BSD `sed` (macOS) has no `\s`**, so `sed -E 's/^\s*file\s*=//'` silently leaves text unstripped and a path-existence check then reports everything missing. Use `perl -ne` / `perl -i -pe` for extraction and literal replacements (`\Q...\E`).
- **In zsh, one unmatched glob aborts the whole command:** `ls -d *Wang* *Himss*` prints nothing but "no matches found" if any single pattern misses. Loop over patterns separately, or filter `ls` output with `grep`.
- **Commit-and-push chains can exceed the Bash tool's 120 s default** (hooks plus the pre-push security checks); pass `timeout` (e.g. 300000). A timed-out command moves to the background and still finishes, so read its output file before retrying, or the retry double-commits.
...
- **TrendMD has 2 halves.** The widget on your own article is *inbound* (others' articles shown to your readers) and proves nothing about the purchase. The $250 buys *outbound* placement into other publishers' widgets. The `rev.trendmd.com/open/...` tracking links are session-bound and return `{"message":"Invalid encrypted payload"}` to `curl`; they must be clicked in-page.
- **In an ad or recommendation network, absence proves nothing.** Sponsored slots are served per impression: at 96541 the author's own page capture carried a Pfizer sponsored item that appeared in none of 3 Playwright captures of the same page the same day. Seeing your item placed proves delivery; not seeing it cannot distinguish "not running" from "not served in this sample". Settle it with the vendor's campaign report or the recommendation API (next bullet), never with a rendered page, and never attach widget screenshots as proof of non-delivery. Check that the report is for your article: the one marketing sent at 96541 was labeled with a different paper's title. This held at 96541 for both paid promotions: the TrendMD campaign and the sponsored tweet were each running while every author-side check showed nothing (2 false negatives out of 2).
- **TrendMD delivery CAN be observed positively, via its recommendation API** (worked 2026-09-23 at 96541). The widget calls `POST https://rev.trendmd.com/ad-slots/<slot-id>/recommendations` with body `{"npis":[],"doctorSpecialties":[],"sourceMetadata":{"title":...,"doi":...},"startTime":<ms>}` and headers `x-revops-version` (e.g. `0.634.0`), `x-revops-source-type: 2`, `x-revops-session-id` (any UUID), `x-revops-source-url`; without the headers it returns 400. Get a slot ID from the host page's resource list (`performance.getEntriesByType('resource')`), then replay the POST from inside that page in Playwright (`browser_evaluate` + `fetch`), varying `sourceMetadata.title` to topical hosts, 1 call/s. Each of the 10 items has `linkingType` (0 = host publisher's organic content, 1 = another publisher's paid campaign), `campaignId`, `title`, `subtitle.content` (byline), `publicationName`. Sample a **non-JMIR** slot (BMJ Innovations: `d4a1ec99-9e42-40c8-b978-0900a2367aae`) and look for your article as `linkingType: 1`. Match loosely: the listing used the pre-copyedit title, so a regex on the published title missed it. **Never request `clickUrl`/`url`**: each registers a billed $1 click. At 96541 this found campaign `6nFy9EFs` running but crossed with another article (our stale title, another paper's first author as byline, title in the journal field), which no report or rendered widget would have shown. **API replay is detection, not evidence**: it uses synthetic host titles, so for anything sent to the vendor, capture a real host page rendering the listing with `ARCHIVED/20260810_IJMR-Copyediting/20260923_trendmd_evidence_capture.py` (fresh context per load; saves widget/item/viewport/full-page PNGs, the response, and meta with URL and UTC time; `--test-campaign '*'` exercises the capture path on any external item). Serving is intermittent: 14/20 at 12:50 UTC, 0/10 at 13:10 UTC the same day. The 519-load run (2026-09-23/24) caught it once, at 03:10 UTC, and the Continuity Trap campaign 8 times, all between 00:00 and 03:42 UTC and none in the 10.5 h before: consistent with daily budget pacing, so run a capture across a UTC midnight. The capture showed the 2 campaigns side by side with their bylines swapped.
- **Headless Chrome is stopped at BMJ's Cloudflare check** ("Just a moment..." title, no widget). Setting an ordinary desktop Chrome `user_agent` on the context passes; headed or off-screen windows also pass. **TrendMD hosts outside JMIR** (verified 2026-09-23): BMJ Innovations, BMJ Open, JAMIA Open (OUP). BMJ Health & Care Informatics, J Med Ethics, BMJ Open Quality, and BMC journals do not carry the widget (`trendmd` absent from the HTML).
...
- `Build and Push Container` **fails on every release** at the SBOM-attach step (`Resource not accessible by integration`); the workflow has no `permissions:` block. Non-required, so it does not block merges. Confirmed again at v6.0.0: it is the `Container Build - Literature Review Tool` run on the **tag** push that fails, while the same workflow passes on the `main` push. Tracked in #565; the `[skip ci]` head-commit problem is #566.
- **Right after a push, `gh pr checks --watch` can return the PREVIOUS head's finished checks** as an instant pass. Wait until check-runs exist on the new commit (`gh api repos/OWNER/REPO/commits/<full-sha>/check-runs --jq .total_count` > 0), then watch, then re-verify per check.
- **`gh run list --commit` needs the full SHA**; a short SHA silently returns nothing. `--branch <name>` works.
- **Verify release assets with `gh api repos/OWNER/REPO/releases/tags/vN.N.N --jq '.assets|length'`.** At v6.0.0 a `secrets_run.py gh release view --json assets ... | grep -v <banner> | tail -1` pipeline printed empty for v4, v5, and v6 although each has 3 assets (PDF, DOCX, HTML, attached by the release-triggered Paper Artifacts run); `tail -1` most likely caught a trailing blank line rather than gh returning nothing. Never pick gh output with `tail -1` after the banner filter.
- **The claude-review workflow runs on PR open only**, not on later pushes, so fixes pushed in response to its findings get CI but no second review.
- **Diff the approved proof before calling `paper.md` synced.** At v6.0.0 the review caught that the 2026-08-19 style sync had missed Table 1 and Table 2 abbreviation footnotes (DIKW, NL undefined); adding them floated Figure 1 onto its own page (fixed by width 86.5% to 75%). Check every table's footnote lines, not just body text and captions.
- **Re-sync contrib by MERGE, not rebase, if `contrib/stharrold` was pushed during the release.** A rebased copy of an already-pushed commit cannot fast-forward the remote, and force-push is off-limits; merge `origin/develop` into `origin/contrib/stharrold` instead (v6.0.0: `07c5387`). Backmerge PR is `release/vN.N.N -> develop`.
- **Check contrib CI before starting a release:** `gh run list --workflow Tests --branch contrib/stharrold --limit 10`. Direct commits do not gate, so Tests sat red for 4 weeks (from `f100969`, 2026-08-19: `abbreviations.json` changed without its tests) and blocked the v6.0.0 integration until fixed in #569.
- **Deterministic version helper:** `uv run python -c "import sys; sys.path.insert(0, '../stharrold-templates'); from release_lib.semver import next_version_from_tag; print(next_version_from_tag(base_branch='origin/main', ref='origin/main'))"`, run with `contrib/stharrold` or `develop` checked out (`base_branch` is diffed three-dot against HEAD; `ref` only scopes the `git describe --tags` lookup). **`ref` must be `origin/main`:** the backmerge merges `release/vN.N.N`, not `main`, so the tagged merge commit is never reachable from `develop` or `contrib`, and `ref='origin/develop'` silently starts from the previous tag (it proposed v5.0.1 after v6.0.0 shipped). The file-pattern bump is still always PATCH for this repo's content; override per the milestone convention and state why in the release PR.
- **Worktrees live at `../yuimedi-paper-20250901.worktrees/<name>`** (`release-vN.N.N`, or `<ts>_<slug>` for work branches). `uv run` inside one creates its own untracked `.venv`, so `git worktree remove` needs `--force`; check `git -C <wt> status --short --untracked-files=no` is empty first.
- **A `paper.md` fix on `release/*` is not rebuilt by CI** (Paper Artifacts triggers only on `main`, `develop`, `contrib/*`). Rebuild locally with `./scripts/build_paper.sh --format all`, then `git checkout HEAD -- multimedia_appendix_*` when the appendix `.md` sources are unchanged: their rebuilds are noise (identical PDF text, pandoc code-highlight CSS churn in HTML). After the merge reaches contrib, CI pushes a `[skip ci]` artifact commit, so `git pull --rebase` before the next contrib push.
