...
- **JMIR tracks tweets per article** at `<article-url>/tweetations` (set the range to All Time). JMIR also posts to Bluesky (`bsky.app/profile/jmirpub.bsky.social`), not only `x.com/jmirpub`; check both plus Tweetations before reporting a sponsored tweet missing.
