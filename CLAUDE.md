...
- **Deterministic version helper:** `uv run python -c "import sys; sys.path.insert(0, '../stharrold-templates'); from release_lib.semver import next_version_from_tag; print(next_version_from_tag(base_branch='origin/main', ref='origin/develop'))"`. It still returns PATCH here (v5.0.1 for v6.0.0); override per the milestone convention and state why in the release PR.
