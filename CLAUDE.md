...
- **Pandoc image sizing in paper.md:** specify only ONE of `{width=X%}` or `{height=Yin}`. Pandoc's LaTeX template auto-injects *both* `keepaspectratio` AND a page-fit secondary dimension (`height=\textheight` or `width=\linewidth`), so aspect ratio is always preserved in the output. The single-dimension rule is about authoring clarity, not distortion prevention. Size square figures by width, portrait figures by height (a modest width on a tall figure blows past page bottom). GitHub's diff view truncates long `\includegraphics` lines after `alt={...}`, so review bots (Copilot, Gemini) can falsely flag `keepaspectratio` as missing; verify the full line in `paper.tex` before acting on those suggestions.
