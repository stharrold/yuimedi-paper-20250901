...
3.  **Validation**: The AI provides a natural language explanation of the SQL logic, and the domain expert validates the query's intent based on this explanation and the results, rather than reviewing the raw SQL code. This human-in-the-loop step is essential because current NL2SQL models are "not yet sufficiently accurate for unsupervised use" in clinical settings [@ziletti2024].
...
4. **Expansion to Unstructured Data**: Acknowledge that while this paper focuses on SQL (structured data), ~80% of healthcare data is unstructured. Future research should explore how the three-pillar framework can provide the necessary governance structure for future expansion into unstructured data via Vector/RAG technologies.
