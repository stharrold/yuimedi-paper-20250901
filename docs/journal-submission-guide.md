# Healthcare Informatics Journal Submission Guide
## YuiQuery Research Paper Publication Strategy

**Purpose**: Comprehensive guide to journal selection and submission for healthcare informatics research on conversational AI platforms.

**Document Version**: 2.0
**Created**: 2025-11-21
**Updated**: 2025-12-11
**Target Papers**: Papers 1-3 (Literature Review, Proof of Concept, Schema Mapping)

---

## Executive Summary

This guide provides detailed information on target journals for publishing YuiQuery Healthcare Analytics research. The strategy balances impact factor, audience reach, open access options, and submission timelines to maximize research dissemination and thought leadership positioning.

**Recommended Primary Target:**
1. **npj Digital Medicine (Nature Portfolio)** - Highest impact (IF 15.1), rapid decisions, open access

**Secondary Targets (Nature Portfolio):**
2. **npj Health Systems** - Health systems research, AI integration
3. **Communications Medicine** - Broad clinical/translational scope

**Tertiary Targets (Traditional):**
4. **JAMIA (Journal of the American Medical Informatics Association)** - High impact, clinical focus
5. **JMIR Medical Informatics** - Open access, rapid publication
6. **Applied Clinical Informatics** - ACI specialty focus on healthcare AI

---

## Tier 0: Nature Portfolio Journals (Primary Targets)

### 1. npj Digital Medicine (PRIMARY TARGET)

**Impact Factor**: 15.1 (2024) - Highest in healthcare informatics
**Publisher**: Nature Publishing Group (Springer Nature)
**Focus**: Digital medicine, AI/ML in healthcare, clinical informatics

**Why This Journal**:
- **2x higher impact factor** than JAMIA (15.1 vs 7.5)
- Median **7 days** to first editorial decision
- Fully open access with high visibility
- Strong fit for NL2SQL and conversational AI research
- Nature brand recognition and credibility

**Submission Details**:
- **URL**: https://www.nature.com/npjdigitalmed/
- **Aims & Scope**: https://www.nature.com/npjdigitalmed/aims
- **Article Types Suitable**:
  - Original Research (our literature review fits here)
  - Perspectives (thought leadership pieces)
- **Review Timeline**: 7 days to first decision, ~15 weeks to publication
- **APC**: ~$3,590 USD / £2,590 / €2,890
- **Open Access**: Yes (fully OA)

**Best Fit**:
- **Paper 1 (Literature Review)**: Strong fit - AI in healthcare analytics
- Emphasize: NL2SQL advancement, healthcare analytics democratization, institutional memory

**Scope Alignment**:
- ✅ Clinical application of AI and machine learning
- ✅ Clinical informatics and digital transformation
- ✅ Healthcare analytics and decision support
- ✅ Natural language processing in clinical settings

---

### 2. npj Health Systems

**Impact Factor**: New journal (launched 2024)
**Publisher**: Nature Publishing Group (Springer Nature)
**Focus**: Health systems research, policy, AI integration

**Why This Journal**:
- Interdisciplinary health systems focus
- AI and informatics integration in health systems
- APC waivers available
- Growing journal with Nature backing

**Submission Details**:
- **URL**: https://www.nature.com/npjhealthsyst/
- **Focus Areas**: Healthcare financing, delivery models, policy interventions, AI integration
- **APC**: Waivers available on ad-hoc basis

**Best Fit**:
- Paper focusing on health system transformation
- Institutional memory and workforce challenges angle

---

### 3. Communications Medicine

**Impact Factor**: 6.3 (2024)
**Publisher**: Nature Publishing Group (Springer Nature)
**Focus**: Clinical, translational, and public health research

**Why This Journal**:
- Broad scope accepts diverse healthcare research
- Active in healthcare AI publications
- Lower APC than npj Digital Medicine
- Q1 ranking

**Submission Details**:
- **URL**: https://www.nature.com/commsmed/
- **Review Timeline**: ~21 weeks to publication
- **APC**: ~$3,490 USD / £2,490 / €2,890
- **Waivers**: Available for qualifying authors

**Best Fit**:
- Backup for npj Digital Medicine rejection
- Broader clinical/translational angle

---

## Tier 1: High-Impact Peer-Reviewed Journals

### 4. JAMIA (Journal of the American Medical Informatics Association)

**Impact Factor**: 7.5 (2024)
**Publisher**: Oxford University Press
**Focus**: Clinical and translational biomedical informatics

**Why This Journal**:
- Premier journal in healthcare informatics
- High citation potential for academic credibility
- Strong clinical and research audience
- AMIA (American Medical Informatics Association) publication

**Submission Details**:
- **URL**: https://academic.oup.com/jamia
- **Author Guidelines**: https://academic.oup.com/jamia/pages/General_Instructions
- **Article Types Suitable**:
  - Original Investigations (Paper 2: Proof of Concept)
  - Systematic Reviews (Paper 1: Literature Review)
  - Focus Articles (Paper 3: Schema Mapping methodology)
- **Review Timeline**: 8-12 weeks peer review
- **Page Limits**:
  - Original research: 5,000 words
  - Systematic reviews: 6,000 words
- **Open Access Option**: Yes (APC: ~$3,500)
- **Citation Format**: Vancouver style

**Best Fit**:
- **Paper 1**: Systematic review article type
- **Paper 2**: Original investigation with empirical validation
- **Paper 3**: Focus article on technical methodology

**Submission Requirements**:
- IRB approval documentation (if applicable)
- PRISMA checklist for systematic reviews
- Data availability statement
- Conflict of interest disclosure
- Author contribution statements

---

### 2. JMIR Medical Informatics

**Impact Factor**: 4.2 (2024)
**Publisher**: JMIR Publications
**Focus**: Medical informatics, eHealth, digital health interventions

**Why This Journal**:
- Open access = maximum visibility
- Rapid review and publication (4-6 weeks)
- Strong online presence and social media reach
- International audience
- Indexed in PubMed, Scopus, Web of Science

**Submission Details**:
- **URL**: https://medinform.jmir.org/
- **Author Guidelines**: https://www.jmir.org/author-instructions
- **Article Types Suitable**:
  - Original Papers (5,000-8,000 words)
  - Viewpoint Papers (2,000-4,000 words)
  - Tutorial/Review (5,000-10,000 words)
- **Review Timeline**: 4-6 weeks
- **Publication Fee (APC)**: $2,495-$3,495
- **Citation Format**: Vancouver style
- **Language**: English only

**Best Fit**:
- **Paper 1**: Tutorial/Review article
- **Paper 2**: Original Paper with usability evaluation
- **Paper 3**: Original Paper on technical implementation

**Advantages**:
- Fully open access (no paywall barriers)
- Multimedia supplements supported
- Interactive figures and demonstrations possible
- Strong social media promotion by publisher

**Requirements**:
- CONSORT-EHEALTH checklist (if applicable)
- Video abstract option
- Author ORCID IDs required

---

### 3. Applied Clinical Informatics (ACI)

**Impact Factor**: 3.9 (2024)
**Publisher**: Thieme (in collaboration with AMIA)
**Focus**: Clinical informatics applications and implementations

**Why This Journal**:
- Specialized in clinical applications of informatics
- Strong practitioner and clinician audience
- Open access journal
- Rapid publication

**Submission Details**:
- **URL**: https://www.thieme.com/aci
- **Author Guidelines**: https://www.thieme.com/aci-instructions
- **Article Types**:
  - Original Research (up to 5,000 words)
  - Brief Research Reports (up to 2,500 words)
  - Systematic Reviews (up to 6,000 words)
- **Review Timeline**: 6-8 weeks
- **APC**: $2,800 (open access)
- **Citation Format**: AMA style

**Best Fit**:
- **Paper 1**: Systematic review
- **Paper 2**: Original research with clinical validation
- **Paper 3**: Brief research report on implementation

**Special Features**:
- PubMed Central indexing
- Emphasis on real-world clinical impact
- Case studies and implementation reports valued

---

## Tier 2: Established International Journals

### 4. International Journal of Medical Informatics (IJMI)

**Impact Factor**: 4.9 (2024)
**Publisher**: Elsevier
**Focus**: Medical informatics, IT in healthcare, clinical decision support

**Submission Details**:
- **URL**: https://www.sciencedirect.com/journal/international-journal-of-medical-informatics
- **Review Timeline**: 10-14 weeks
- **Open Access Option**: Yes (APC: ~$3,200)
- **Article Types**: Full-length articles, short communications, reviews
- **Word Limit**: 6,000 words (full articles)

**Best Fit**: All three papers suitable

**Strengths**:
- High impact factor
- International scope
- Broad healthcare informatics audience

**Considerations**:
- Longer review timeline
- Higher APC costs
- Elsevier publication (consider institutional policies)

---

### 5. BMC Medical Informatics and Decision Making

**Impact Factor**: 3.5 (2024)
**Publisher**: BMC (BioMed Central) / Springer Nature
**Focus**: Medical informatics, decision support, health information systems

**Submission Details**:
- **URL**: https://bmcmedinformdecismak.biomedcentral.com/
- **Review Timeline**: 6-10 weeks
- **APC**: $2,290 (fully open access)
- **Article Types**: Research articles, software articles, reviews, methodology
- **Word Limit**: No strict limit, typically 4,000-8,000 words

**Best Fit**: All three papers, especially Paper 2 (decision support focus)

**Advantages**:
- Fully open access
- No page/figure charges beyond APC
- Fast publication after acceptance
- Indexed in PubMed Central

---

### 6. Healthcare Informatics Research (HIR)

**Impact Factor**: 2.8 (2024)
**Publisher**: Korean Society of Medical Informatics
**Focus**: Healthcare informatics research and applications

**Submission Details**:
- **URL**: https://www.e-hir.org/
- **Review Timeline**: 8-12 weeks
- **APC**: Free to publish, free to read (Diamond open access)
- **Article Types**: Original articles, reviews, case reports

**Advantages**:
- No publication fees
- Open access
- Rapid online publication

**Considerations**:
- Lower impact factor
- Less well-known internationally
- Still indexed in PubMed, Scopus

---

## Specialized Healthcare Analytics Journals

### 7. Health Data Science

**Publisher**: Oxford University Press (OUP) / Chinese Academy of Sciences
**Focus**: Data science methods in health and biomedicine
**Status**: Newer journal (launched 2021)

**URL**: https://spj.science.org/journal/hds
**Best Fit**: Paper 2 (algorithm development), Paper 3 (schema mapping)

**Strengths**:
- Emphasis on computational methods
- Open access
- Growing reputation

---

### 8. Journal of Healthcare Informatics Research (JHIR)

**Publisher**: Springer
**Focus**: Informatics research methods and applications

**URL**: https://www.springer.com/journal/41666
**Best Fit**: All papers
**Timeline**: 8-12 weeks

---

### 9. Journal of Biomedical Informatics

**Impact Factor**: 4.5 (2024)
**Publisher**: Elsevier
**Focus**: Biomedical informatics theory and applications

**URL**: https://www.sciencedirect.com/journal/journal-of-biomedical-informatics
**Best Fit**: Papers 2 and 3 (technical focus)

---

## Preprint and Repository Options

### arXiv (Computer Science / Health Informatics) - PRIMARY PREPRINT OPTION

**Focus**: CS, AI/ML, NLP, informatics preprints

**Details**:
- **URL**: https://arxiv.org/submit
- **Cost**: FREE
- **Timeline**: 24-72 hours for posting (after moderation)
- **DOI**: Provided immediately
- **Indexing**: Google Scholar, Semantic Scholar, major databases
- **Requirement**: Endorsement required for first-time submitters

**Recommended Categories for Paper 1 (NL2SQL Healthcare Analytics)**:
- **Primary**: cs.CL (Computation and Language) - NL2SQL focus
- **Cross-list**: cs.DB (Databases) - SQL generation, data warehouses
- **Cross-list**: cs.HC (Human-Computer Interaction) - user interfaces, democratization
- **Cross-list**: cs.CY (Computers and Society) - healthcare applications

**Why arXiv over medRxiv**:
- arXiv accepts narrative reviews, analytical frameworks, and position papers
- medRxiv explicitly excludes narrative reviews (only accepts systematic reviews with meta-analysis)

**Strategy**:
- Post preprint of Paper 1 to arXiv after completion
- Establishes priority and thought leadership
- Does not preclude journal submission (npj Digital Medicine allows preprints)

**Best Fit**: All three papers (Paper 1 especially given narrative review format)

---

### medRxiv (Healthcare Sciences Preprints) - NOT RECOMMENDED FOR PAPER 1

**Purpose**: Rapid dissemination before peer review

**Details**:
- **URL**: https://www.medrxiv.org/submit-a-manuscript
- **Cost**: FREE
- **Timeline**: 24-72 hours for posting
- **DOI**: Provided immediately
- **Indexing**: Google Scholar, some databases

**Scope Limitations** (as of Dec 2025):
- **Accepts**: Original research, systematic reviews with meta-analysis, scoping reviews with meta-analysis, methodological research, clinical protocols
- **Does NOT accept**: Narrative reviews, hypotheses without data, opinion pieces, editorials, case reports

**Paper 1 Status**: **NOT ELIGIBLE** - Paper 1 is a narrative review with an analytical framework, which medRxiv explicitly excludes.

**Recommended for**: Papers 2 and 3 only (if they include empirical primary research)

---

## Conference Proceedings (Complementary Strategy)

### AMIA Annual Symposium

**Organization**: American Medical Informatics Association
**Focus**: Medical informatics research and practice

**Details**:
- **URL**: https://amia.org/education-events/amia-2025-annual-symposium
- **Abstract Deadline**: Typically March-April
- **Full Paper Option**: Yes (published in JAMIA Supplements)
- **Presentation Types**: Podium, poster, panel, workshop

**Strategy**: Submit abstracts for all three papers to annual symposium

---

### HIMSS Global Health Conference

**Organization**: Healthcare Information and Management Systems Society
**Focus**: Health IT, interoperability, analytics

**Details**:
- **URL**: https://www.himssconference.org/
- **Proposal Deadline**: Typically June-July (for following year)
- **Publication**: Conference proceedings, not peer-reviewed journal

**Strategy**: Present findings, build industry connections, not primary publication venue

---

## Submission Timeline Strategy

### Paper 1: Literature Review (Deadline: 2025-09-30)

**Primary Path**:
1. **Pre-submission**: Send inquiry to npj Digital Medicine (see `docs/npj-digital-medicine-inquiry-email.md`)
2. **Week 1-2**: Submit to **arXiv** preprint (cs.CL primary, cross-list cs.DB, cs.HC, cs.CY)
3. **Week 2-3**: Prepare npj Digital Medicine submission (pending inquiry response)
4. **Week 4**: Submit to **npj Digital Medicine** as Article or Perspective
5. **Week 5+**: 7-day first editorial decision, ~178 days to acceptance if peer reviewed

**Note**: Paper 1 is a narrative review with analytical framework. NOT eligible for medRxiv (narrative reviews excluded). arXiv is the appropriate preprint server.

**Backup Path** (if npj Digital Medicine declines):
- Submit to **JMIR Medical Informatics** (4-6 week review) - broad scope
- Submit to **Communications Medicine** (Nature Portfolio alternative)
- Submit to **BMC Medical Informatics** (6-10 week review)

---

### Paper 2: Proof of Concept (Deadline: 2026-01-10)

**Primary Path**:
1. **Jan 2026**: Submit to **arXiv** and **medRxiv** as preprints (Paper 2 qualifies for medRxiv if it includes empirical primary research)
2. **Jan-Feb 2026**: Prepare submission for **JMIR Medical Informatics**
3. **Feb 2026**: Submit to JMIR
4. **Mar-Apr 2026**: Review and revision
5. **May 2026**: Publication (if accepted)

**Alternative Targets**:
- **ACI**: If clinical validation emphasized
- **JAMIA**: If empirical findings strong
- **IJMI**: If broader international audience desired

---

### Paper 3: Schema Mapping (Deadline: 2026-03-15)

**Primary Path**:
1. **Mar 2026**: Submit to **arXiv** preprint (CS.DB, CS.HC categories)
2. **Apr 2026**: Submit to **Journal of Biomedical Informatics** or **ACI**
3. **May-Jul 2026**: Review process
4. **Aug-Sep 2026**: Publication

---

## Submission Requirements Checklist

### General Requirements (All Journals)

- [ ] Author affiliation and contact information
- [ ] Author ORCID IDs
- [ ] Abstract (structured for most journals)
- [ ] Keywords (5-8 typically)
- [ ] Conflict of interest statement
- [ ] Author contribution statements
- [ ] Funding acknowledgment
- [ ] Data availability statement
- [ ] Ethics approval (IRB determination)

### Systematic Review Specific (Paper 1)

- [ ] PRISMA 2020 checklist
- [ ] PRISMA flow diagram
- [ ] Search strategy documentation
- [ ] Protocol registration (PROSPERO - optional but recommended)
- [ ] Risk of bias assessment

### Empirical Research Specific (Paper 2)

- [ ] Methods section with reproducibility details
- [ ] Statistical analysis plan
- [ ] Data sharing statement
- [ ] Code availability (GitHub repository)
- [ ] IRB approval or exemption documentation

### Technical Implementation (Paper 3)

- [ ] System architecture diagrams
- [ ] Implementation details
- [ ] Performance benchmarks
- [ ] Limitations and future work section

---

## Open Access Funding Strategy

### Publication Cost Estimates

| Journal | APC (USD) | Payment Timeline | Waiver/Discount Available? |
|---------|-----------|------------------|---------------------------|
| **npj Digital Medicine** | **$3,590** | Upon acceptance | Limited waivers |
| npj Health Systems | TBD | Upon acceptance | Waivers available |
| Communications Medicine | $3,490 | Upon acceptance | Limited waivers |
| JAMIA | $3,500 | Upon acceptance | Sometimes for members |
| JMIR Med Inform | $2,495-$3,495 | Upon acceptance | Limited waivers for LMICs |
| ACI | $2,800 | Upon acceptance | No |
| IJMI | $3,200 | Upon acceptance | No |
| BMC Med Inform | $2,290 | Upon acceptance | Institutional agreements |
| medRxiv | FREE | N/A | N/A |
| arXiv | FREE | N/A | N/A |

### Budget Allocation

From project-management/budget/budget-final.json:
- Review and publication costs budgeted
- Open access fees should be allocated per paper
- Estimated total: $6,000-$10,000 for 3 papers if all open access

### Funding Acknowledgment

Standard text:
```
This research was supported by Yuimedi in the development of
YuiQuery Healthcare Analytics Platform. The funder had no role
in study design, data collection and analysis, decision to
publish, or preparation of the manuscript.
```

---

## Journal Selection Decision Matrix

### Selection Criteria Scoring (Updated December 2025)

| Criterion | Weight | npj Digital Med | Comms Med | JAMIA | JMIR | ACI |
|-----------|--------|-----------------|-----------|-------|------|-----|
| Impact Factor | 25% | **10/10** | 8/10 | 9/10 | 6/10 | 6/10 |
| Review Speed | 20% | **10/10** | 5/10 | 5/10 | 9/10 | 7/10 |
| Open Access | 20% | **10/10** | 10/10 | 6/10 | 10/10 | 10/10 |
| Audience Fit | 20% | **10/10** | 8/10 | 9/10 | 8/10 | 8/10 |
| Cost | 15% | 5/10 | 5/10 | 5/10 | 6/10 | 6/10 |
| **Total Score** | | **9.25** | **7.4** | **7.2** | **7.9** | **7.5** |

**Recommendation Priority (Updated)**:
1. **npj Digital Medicine** (9.25) - **PRIMARY TARGET** - Highest impact, fastest decisions, perfect scope fit
2. **JMIR Medical Informatics** (7.9) - Backup #1 - Fast review, good balance
3. **ACI** (7.5) - Backup #2 - Clinical applications focus
4. **Communications Medicine** (7.4) - Backup #3 - Nature Portfolio alternative
5. **JAMIA** (7.2) - Backup #4 - High prestige, longer timeline

---

## Risk Mitigation for Journal Submission

### Common Rejection Reasons

1. **Out of Scope**: Ensure manuscript aligns with journal's stated focus areas
2. **Insufficient Novelty**: Emphasize unique contributions (3-pillar framework, empirical validation)
3. **Methodological Issues**: Follow PRISMA, CONSORT, or other reporting guidelines
4. **Poor Writing Quality**: Professional editing recommended before submission
5. **Incomplete Data Sharing**: Prepare supplementary materials in advance

### Mitigation Strategies

- **Pre-submission inquiries**: Contact editors to confirm fit
- **Author guidelines compliance**: Follow formatting meticulously
- **Internal review**: Peer review by backup developer and SMEs before submission
- **Manuscript editing**: Budget for professional editing service if needed
- **Parallel preparation**: Prepare backups for alternative journals while awaiting decision

---

## Post-Publication Promotion Strategy

### After Acceptance

1. **Institutional Press Release**: Coordinate with Yuimedi marketing
2. **Social Media**: LinkedIn, Twitter/X announcements
3. **Conference Presentations**: HIMSS, AMIA, regional HIE conferences
4. **Blog Posts**: 3-5 blog posts per paper highlighting key findings
5. **Webinar Series**: Present findings to healthcare analytics community
6. **ResearchGate/Academia.edu**: Upload preprints and accepted manuscripts

### Metrics Tracking

- Citation count (Google Scholar, Scopus, Web of Science)
- Altmetric score (social media mentions, news coverage)
- Download statistics
- Speaking invitation rate
- Inbound business inquiries

---

## Special Considerations

### Predatory Journal Avoidance

**Red Flags to Avoid**:
- Unsolicited submission invitations
- Promises of rapid publication (< 1 week)
- Unclear peer review process
- Not indexed in PubMed/Scopus/Web of Science
- Spelling/grammar errors on journal website
- Unclear or hidden APCs

**Verification Tools**:
- Beall's List awareness (archived, but still useful)
- Think.Check.Submit checklist
- Directory of Open Access Journals (DOAJ) listing

### Journal Policy Compliance

**Yuimedi Considerations**:
- **Proprietary Information**: Ensure no trade secrets disclosed
- **Intellectual Property**: Confirm open access doesn't limit commercialization
- **Author Affiliations**: Accurate representation of Yuimedi vs. independent research
- **Competing Interests**: Transparent disclosure of commercial interests

---

## Resources and Tools

### Manuscript Preparation

- **Endnote/Zotero/Mendeley**: Citation management
- **Overleaf**: LaTeX manuscript preparation (for technical papers)
- **Grammarly/Pro Writing Aid**: Grammar and style checking
- **PRISMA-P/PRISMA 2020**: Systematic review reporting
- **Equator Network**: Reporting guideline resources

### Journal Metrics

- **Clarivate Journal Citation Reports**: Impact factors
- **Scopus Journal Metrics**: CiteScore, SNIP, SJR
- **Google Scholar Metrics**: h5-index, h5-median
- **DOAJ**: Open access journal directory

### Submission Platforms

- **Editorial Manager**: Used by many Elsevier journals
- **ScholarOne**: Used by Oxford journals (JAMIA)
- **Open Journal Systems**: Used by some open access journals

---

## Conclusion and Recommendations

### Recommended Strategy for YuiQuery Research

**Immediate Actions (Q4 2025)**:
1. Submit Paper 1 to medRxiv as preprint upon completion
2. Prepare JAMIA submission with full PRISMA compliance
3. Begin JMIR Medical Informatics submission preparation as backup

**Q1 2026 Actions**:
1. Submit Paper 2 to arXiv and medRxiv
2. Target JMIR Medical Informatics for Paper 2 peer review
3. Present Paper 1 findings at conferences (if accepted)

**Q2 2026 Actions**:
1. Submit Paper 3 to arXiv
2. Target Journal of Biomedical Informatics or ACI
3. Leverage Paper 1 citations in Papers 2 and 3 submissions

### Success Metrics

- **3 preprints** published (medRxiv/arXiv) - immediate thought leadership
- **2+ peer-reviewed publications** within 12 months
- **Speaking invitations** from at least 2 major conferences
- **Citation by others** within 6 months of preprint publication
- **Inbound inquiries** from healthcare organizations

---

## Document Control

**Version**: 1.0
**Last Updated**: 2025-11-21
**Next Review**: 2026-01-01
**Owner**: Research Publication Team

**Related Documents**:
- project-management.md (Publication venue options)
- paper.md (Research manuscripts)
- docs/references/README.md (Citation management)

---

*This guide provides comprehensive journal submission strategy for maximizing impact and reach of YuiQuery Healthcare Analytics research.*
