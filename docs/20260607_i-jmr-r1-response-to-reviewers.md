...
NOTE FOR SUBMISSION: The editor requires point-by-point responses pasted directly into the notification email, with no separate file and no formatting (bold/italics). The text below is plain and paste-ready. A clean revised manuscript and a tracked-changes version are uploaded as instructed.

---

We thank the editor and both reviewers for their constructive and detailed comments. The revision strengthens the manuscript's engagement with recent peer-reviewed literature, adds explicit definitions of the three core constructs, addresses the scope and human-in-the-loop efficacy concerns with empirical evidence, and brings the formatting into compliance. In total we added 14 peer-reviewed references (2016-2026). Each comment is addressed below; section names refer to the revised manuscript.
...
- Regarding the requirement that all subheadings fall under Introduction, Methods, Results, and Discussion: this manuscript is a Viewpoint. Per JMIR's instructions for authors and article-type definitions, Viewpoints use an unstructured format in which descriptive section headers are recommended, and headings such as "Methods" and "Results" are not permitted. We have therefore used descriptive headers throughout, as the Viewpoint article type requires. We would be glad to adjust further if the editor prefers, recognizing that an IMRD structure would entail re-designating the article type.
