...
class TestGeminiAnalyzerInit:
    """Tests for GeminiAnalyzer initialization."""
    ...
    def test_init_with_api_key_enables_api(self, temp_cache_dir: Path) -> None:
        ...
        with patch("google.genai.Client"):
            analyzer = GeminiAnalyzer(api_key="test_key", cache_dir=temp_cache_dir)
    ...
    def test_init_creates_cache_directory(self, temp_cache_dir: Path) -> None:
        ...
        analyzer = GeminiAnalyzer(cache_dir=cache_dir)
    ...
    def test_init_accepts_custom_model(self, temp_cache_dir: Path) -> None:
        ...
        analyzer = GeminiAnalyzer(model="gemini-1.5-pro", cache_dir=temp_cache_dir)
        assert analyzer.model == "gemini-1.5-pro"


class TestGeminiAnalyzerFallback:
    ...
    def test_extract_themes_fallback_returns_themes(
        ...
        with patch.dict("os.environ", {}, clear=True):
            analyzer = GeminiAnalyzer(cache_dir=temp_cache_dir)  # No API key
...
class TestGeminiAnalyzerCaching:
    ...
    def test_cache_expires_after_ttl(self, temp_cache_dir: Path) -> None:
        ...
        analyzer = GeminiAnalyzer(cache_dir=temp_cache_dir, cache_ttl_days=7)
...
class TestGeminiAnalyzerHelpers:
...
class TestGeminiAnalyzerIntegration:
    """Integration tests for Gemini API (requires API key)."""

    def test_real_gemini_theme_extraction(
        self, temp_cache_dir: Path, sample_papers: list[Paper]
    ) -> None:
        """extract_themes uses real Gemini API (integration test)."""
        import os
        ...
        analyzer = GeminiAnalyzer(api_key=api_key, cache_dir=temp_cache_dir)
        themes = analyzer.extract_themes(sample_papers, max_themes=3)
    ...
    def test_real_gemini_synthesis(self, temp_cache_dir: Path, sample_papers: list[Paper]) -> None:
        """generate_synthesis uses real Gemini API (integration test)."""
        import os
